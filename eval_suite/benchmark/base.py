...
from eval_suite.benchmark.stat import BaseStat, EvalStatBase
...
class BenchmarkBase[
...
](MetricBase[Input, Output, Result, Config]):
    """
    Base class for all benchmarks.

    Benchmark is basically a "runnable" metric, with dataset, base_path, etc.
    """
    ...
    def to_stat(self, groups: EvalResultGroups[Result], base: BaseStat) -> Stat:
        ...
        raise NotImplementedError(
            "`to_stat` not implemented. Please implement it in the subclass"
        )
...
class BenchmarkExcutor:  # Type-free since we don't really care about concrete types
    def __init__(self, benchmark: BenchmarkBase, client: ClientBase) -> None:
        ...
        self._result_group._config = self._ben.eval_config
    ...
    async def _input_stream(self):
        ...
        n_samples = self._ben.eval_config.n_samples
        ...
        max_n_samples = self._ben.eval_config.max_n_samples
    ...
    async def run(self) -> BaseModel | None:
        ...
        if (
            stat_path := self._eval_path / self._ben.benchmark_config.stat_file
        ).exists():
