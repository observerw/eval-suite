...
from typing import Any, ClassVar, NamedTuple, Self, TypeVar, cast, final
...
from eval_suite.benchmark.schema import EvalID, EvalInputBase, EvalOutputBase, InputID
from eval_suite.benchmark.stat import BaseStat, EvalStatBase
from eval_suite.benchmark.utils import dump_json, resolve_methods
...
class BenchmarkBase[
    ...
    def to_output(self, generation: Message, input: Input) -> Output:
        ...
        Raises:
            BaseException: If the conversion fails.
        ...
        raise NotImplementedError(
            "`to_output` not implemented. Please implement it in the subclass"
        )
    ...
    def to_result_sync(
        ...
        raise NotImplementedError(
            f"`to_result*` not implemented. Please ensure exactly one of {', '.join(_to_result_order)} is implemented in the subclass"
        )
    ...
    async def to_result(
        ...
        match resolve_methods(self.__class__, _to_result_order, base=BenchmarkBase):
            case "to_result":
            ...
            case _:
                raise NotImplementedError(
                    f"Missing implementation for {_to_result_order}"
                )
        return rets
...
class BenchmarkExcutor:  # Type-free since we don't really care about concrete types
    ...
    @property
    def _to_result_method_resolved(self) -> str | None:
        """The version of `to_result*` to use."""
